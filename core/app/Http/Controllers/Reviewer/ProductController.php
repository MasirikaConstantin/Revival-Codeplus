...
class ProductController extends Controller
{
    public function pendingProducts()
    {
        ...
        $products = $this->productData('pending', true)->whereIn('assigned_to', [0, auth('reviewer')->id()])->latest()->paginate(getPaginate());
        return view('reviewer.product.list', compact('pageTitle', 'products'));
    }

    public function awaitingProducts()
    {
        ...
        $products = Product::whereIn('sub_category_id', $reviewer->subcategories ?? [])->where(function ($q) {
            ...
            ->where('status', '!=', Status::PRODUCT_HARD_REJECTED)->latest()->paginate(getPaginate());
    ...
    public function assignedProducts()
    {
        ...
        $products = $this->productData(null, true)->where('assigned_to', auth('reviewer')->id())->latest()->paginate(getPaginate());
    ...
    public function updatedProducts()
    {
        ...
        $products = $this->ownProductData(null, true)->where('product_updated', Status::ENABLE)->latest()->paginate(getPaginate());
        return view('reviewer.product.list', compact('pageTitle', 'products'));
    }
