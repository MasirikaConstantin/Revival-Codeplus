<div class="product-sidebar">
    ...
    <div class="product-sidebar__item-wrappper">
                ...
                <ul class="text-list text-list--category">
                    ...
                    @foreach ($categories as $category)
                        <li class="text-list__item {{ request()->category == $category->id ? 'active' : '' }}">
                            <a href="{{ appendQuery('category', $category->id) }}" class="text-list__link">
                                {{ __(@$category->name) }} </a>
                        </li>
                    @endforeach
                </ul>
        ...
        <form class="form-sidebar">
                    ...
                    <ul class="text-list">
                        <li class="text-list__item">
                            <div class="form--radio">
                                <input class="form-check-input" type="radio" name="rating" value="all"
                                       id="rating_all" @checked(request()->rating == 'all') />
            ...
            <button type="submit" class="btn btn-outline--base w-100 btn--sm"><i class="icon-Filter"></i>
                @lang('Filter')</button>
        </form>
    </div>
    @php
