...
function appUrl()
{
	// Compatible avec WAMP - détection du schéma HTTP/HTTPS
...
if ($action == 'result') {
	...
	// Gestion des erreurs cURL pour WAMP
	if ($curlErrno || $result === false) {
		$response = [
			'error' => 'error',
			'message' => 'Erreur de connexion au serveur de licence. Vérifiez votre connexion Internet. Erreur cURL: ' . ($curlError ? $curlError : 'Erreur inconnue')
		];
	} else {
		...
		if (json_last_error() !== JSON_ERROR_NONE) {
			$response = [
				'error' => 'error',
				'message' => 'Réponse invalide du serveur de licence. Réponse reçue: ' . substr($result, 0, 200)
			];
		}
	}
